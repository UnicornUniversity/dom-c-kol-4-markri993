...
// Vygenerujeme náhodný věk v intervalu <min, max> a z něj spočítáme birthdate.
...
const birthdate = new Date(birthTime).toISOString();
    ...
    const key = `${name}|${surname}|${birthdate}|${workload}|${gender}`;
    if (usedKeys.has(key)) {
      i--;
      continue;
    }
...
export function getEmployeeStatistics(employees) {
  ...
  const minAge = total ? Math.round(Math.min(...ages) - 1e-9) : 0;
  const maxAge = total ? Math.round(Math.max(...ages) - 1e-9) : 0;

  // Medián věku
  const medianAge = total ? Math.round(median(ages) - 1e-9) : 0;
